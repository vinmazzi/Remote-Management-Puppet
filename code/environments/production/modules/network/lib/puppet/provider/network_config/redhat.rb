...
Puppet::Type.type(:network_config).provide(:redhat) do
  ...
  def self.format_file(filename, providers)
    ...
    # Alterado por VINICIUS MAZZI, A troca de ips n estava funciona quando colocava o resource resources pra deletar o que tava errado
    if providers.length > 2 
      raise Puppet::DevError,
            "Unable to support multiple interfaces [#{providers.map(&:name).join(',')}] in a single file #{filename} #{providers}"
    elsif providers.length == 2
      tmp_providers = providers
      providers = [tmp_providers[1]]
    end
